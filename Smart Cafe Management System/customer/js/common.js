...
window.SmartCafeCommon = {
    ...
    getCart() {
        try {
            const cart = JSON.parse(
                localStorage.getItem('smartCafeCart') || '[]'
            );
    ...
    getOrders() {
        try {
            const orders = JSON.parse(
                localStorage.getItem('smartCafeOrders') || '[]'
            );
    ...
    itemUnitPrice(item) {
        ...
        const menu =
            ...
            Array.isArray(window.SmartCafeData.menu)
        ...
        if (menuItem && Array.isArray(menuItem.sizes)) {
            const sizeOption = menuItem.sizes.find(
                option => String(option[0]) === size
            );

            sizeCharge = sizeOption
                ? Number(sizeOption[1])
                : 0;
        } else {
            sizeCharge =
        ...
        (
            item && Array.isArray(item.extras)
                ? item.extras
                : []
        ).forEach(extra => {
    ...
    cartTotal(cart = this.getCart()) {
        return cart.reduce((sum, item) => {
            ...
            return (
                sum +
                this.itemUnitPrice(item) * quantity
            );
        }, 0);
    },
